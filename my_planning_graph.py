...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if all(a_n in self.s_levels[level] for a_n in  a_node.prenodes):
                for node_s in self.s_levels[level]:
                    if node_s in a_node.prenodes:
                        a_node.parents.add(node_s)
                        node_s.children.add(a_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e1 in node_a2.action.effect_add:
            for e2 in node_a1.action.effect_rem:
                if e1 == e2:
                    return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e1 in node_a2.action.effect_rem:
            for e2 in node_a1.action.precond_pos:
                if e1 == e2:
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for a1 in node_s1.parents:
            if node_s1 in a1.effnodes and node_s2 in a1.effnodes:
                return False
                
        for a2 in node_s2.parents:
            if node_s1 in a2.effnodes and node_s2 in a2.effnodes:
                return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            for idx, level_s in enumerate(self.s_levels):
                if PgNode_s(goal,True) in level_s:
                    level_sum += idx
                    break
